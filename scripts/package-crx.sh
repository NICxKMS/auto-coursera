...
usage() {
    ...
    echo "  $0 -v <version> -k extension-key.pem"
    echo "  $0 -v <version> -k extension-key.pem -s extension/dist -o releases/"
    echo ""
...
if [[ ! "$VERSION" =~ ^[0-9]+\.[0-9]+\.[0-9]+(\.[0-9]+)?$ ]]; then
    ...
    echo -e "${YELLOW}Expected format: X.Y.Z or X.Y.Z.W (e.g., 1.9.1)${NC}"
    exit 1
fi
...
echo "  4. Prod:    Browser update checks stay on https://autocr.nicx.me/updates.xml (static file)"
echo ""
