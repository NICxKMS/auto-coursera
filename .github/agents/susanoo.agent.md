...
|---|---|
| Backend logic, message handlers, API integration, validation, data management | Frontend components, styling, client-side UI (delegate to `@kagutsuchi`) |
| Run project validation commands | Architecture decisions (consult `@minerva`) |
| Read and search the full codebase | Infrastructure: deployment, CI config (delegate to `@maat`) |
